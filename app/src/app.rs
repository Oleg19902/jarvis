    ...
    'wake_word: loop {
        ...
        match listener::data_callback(&frame_buffer) {
            Some(keyword_index) => {
                ...
                if let Some(greet_sound) =
                    config::ASSISTANT_GREET_PHRASES.choose(&mut rand::thread_rng())
                {
                    audio::play_sound(&sounds_directory.join(format!("{}.wav", greet_sound)));
                } else {
                    warn!("No greet sounds configured");
                }
